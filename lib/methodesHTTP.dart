...
//pour chaque heureDisponible, ajoute un objet Map<String, dynamic> à la liste_dispos à envoyer dans le server
  utilisateur.disponibiliteSemaine.
      forEach((heureDisponible) => {
            listeDispos.add(heureDisponible.toMap())
          });
  ...
  listeDispos
      .forEach((heureDisponible) => {listeDisposString.add(jsonEncode(heureDisponible))});
 ...
 //Récupère une liste de ligne de donnée
  ...
  userEntrainements.forEach((entrainement){
    utilisateur.listeEntrainements.add(new Entrainement(nomEntrainement: entrainement['nom'], intensite: entrainement['intensite'],
     plageHoraire: new HeureDisponible(jour : entrainement['dispoJour'], debut : entrainement['dispoDebut'], fin : entrainement['dispoFin'],  ),exercices: new List<Exercice>()));
  });
